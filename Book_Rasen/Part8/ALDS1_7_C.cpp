...
void walkAsPreorder(int id) {
    if (id == -1) return;
    const auto& node = tree[id];
    printId(node.id);
    if (node.left != -1) {
        walkAsPreorder(node.left);
    }
    if (node.right != -1) {
        walkAsPreorder(node.right);
    }
}

void walkAsInorder(int id) {
    if (id == -1) return;
    const auto& node = tree[id];
    if (node.left != -1) {
        walkAsInorder(node.left);
    }
    printId(node.id);
    if (node.right != -1) {
        walkAsInorder(node.right);
    }
}

void walkAsPostorder(int id) {
    if (id == -1) return;
    const auto& node = tree[id];
    if (node.left != -1) {
        walkAsPostorder(node.left);
    }
    if (node.right != -1) {
        walkAsPostorder(node.right);
    }
    printId(node.id);
}
...
int main(){
    ...
    cout << "Preorder" << endl;
    walkAsPreorder(rootId);
    cout << endl;

    cout << "Inorder" << endl;
    walkAsInorder(rootId);
    cout << endl;

    cout << "Postorder" << endl;
    walkAsPostorder(rootId);
    cout << endl;
